...
data_sample["time_of_day"] = pd.cut(
    data_sample["hour"],
    bins=[-1, 6, 10, 14, 18, 24],
    labels=["night", "morning", "noon", "afternoon", "evening"],
)

data_map = data_sample.loc[
    (data_sample["hour"] == hour) & (data_sample["day_of_week"] == day)
]
data_map_list_cluster1 = data_map[["Lat", "Lon"]][
    data_map["labels_kmeans"] == 0
].values.tolist()
data_map_list_cluster2 = data_map[["Lat", "Lon"]][
    data_map["labels_kmeans"] == 1
].values.tolist()
data_map_list_cluster3 = data_map[["Lat", "Lon"]][
    data_map["labels_kmeans"] == 2
].values.tolist()
data_map_list_cluster4 = data_map[["Lat", "Lon"]][
    data_map["labels_kmeans"] == 3
].values.tolist()
data_map_list_cluster5 = data_map[["Lat", "Lon"]][
    data_map["labels_kmeans"] == 4
].values.tolist()
...
for point in range(0, size_map_list_1):
    folium.CircleMarker(
        data_map_list_cluster1[point], radius=2, color="blue", fill_color="lightblue",
    ).add_to(m_p)

# second claster
for point in range(0, size_map_list_2):
    folium.CircleMarker(
        data_map_list_cluster2[point], radius=2, color="green", fill_color="lightgreen",
    ).add_to(m_p)

# third claster
for point in range(0, size_map_list_3):
    folium.CircleMarker(
        data_map_list_cluster3[point], radius=2, color="purple", fill_color="purple",
    ).add_to(m_p)

# fourth claster
for point in range(0, size_map_list_4):
    folium.CircleMarker(
        data_map_list_cluster4[point], radius=2, color="orange", fill_color="orange",
    ).add_to(m_p)

# fifth claster
for point in range(0, size_map_list_5):
    folium.CircleMarker(
        data_map_list_cluster5[point], radius=2, color="red", fill_color="red",
    ).add_to(m_p)
...
fig_pie = px.pie(
    data_frame=data_time,
    values="week",
    names=data_time.index,
    color_discrete_sequence=px.colors.sequential.RdBu_r,
    title="Number of rides per time of the day",
)


### line chart
# operation on data
ride = data_sample.groupby(["week", "hour", "day_of_week", "day_Week_num"])[
    "Base"
].count()
...
fig_line = px.line(
    ...
    markers=True,
    color_discrete_sequence=px.colors.sequential.RdBu_r,
    title="Average number of rides per day and hour",
    labels=dict(
        day_of_week="Day of week", Ride="Averge number of rides", hour="Hour, h"
    ),
)
